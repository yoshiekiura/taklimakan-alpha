...
def calculatePriceAndVolumeRange2(pair, dateList):
    ...
    for date in dateList:
        if date not in volumes.keys():
            ...
            result = False
            # If prices are missing, save a special (negative) value
            saveAnalyticsValue(pairToStr(pair), date, "1", -1)
            saveAnalyticsValue(pairToStr(pair), date, "2", -1)
    return result
...
def getExtrapolatedAssetPrice(pairStr, date):
    for i in range(maxDataGap):
        ...
        if (len(assetPrices) != 0) and (assetPrices[0] != -1):
            if (i != 0):
